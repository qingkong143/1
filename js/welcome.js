...
function showWelcome() {
    ...
    let dist = getDistance(114.804809,24.902492, ipLocation.data.lng, ipLocation.data.lat);
...
// 根据获取的IP地址显示欢迎信息
function showWelcome() {
    let dist = getDistance(121.413921, 31.089290, ipLocation.data.lng, ipLocation.data.lat);
    let pos = ipLocation.data.country;
    let ip = ipLocation.ip;
    let posdesc;
    ...
    let timeChange;
    let date = new Date();
    if (date.getHours() >= 5 && date.getHours() < 11) timeChange = "<span>🌤️ 早上好啊，一日之计在于晨</span>";
    else if (date.getHours() >= 11 && date.getHours() < 13) timeChange = "<span>☀️ 中午好，记得午休喔~</span>";
    else if (date.getHours() >= 13 && date.getHours() < 17) timeChange = "<span>🕞 下午好，饮茶先啦！</span>";
    else if (date.getHours() >= 17 && date.getHours() < 19) timeChange = "<span>🚶‍♂️ 要下班咯，记得按时吃饭呀~</span>";
    else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>🌙 晚上好，夜生活嗨起来！</span>";
    else timeChange = "夜深啦，早点休息，少熬夜哦";

    try {
        // 创建用于显示IP地址的<span>元素，并设置样式和内容
        const ipAddressElement = document.createElement('span');
        ipAddressElement.style.cssText = 'font-size: 12px; word-wrap: break-word;';
        ...
        welcomeInfoElement.innerHTML =
            `欢迎来自 <b><span style="color: var(--kouseki-ip-color);font-size: var(--kouseki-gl-size)">${pos}</span></b> 的盆友<br>${posdesc}🍂<br>当前位置距清空约 <b><span style="color: var(--kouseki-ip-color)">${dist}</span></b> 公里！<br>您的IP地址为：<b><span style="font-size: 14px;">${ip}</span></b><br>${timeChange} <br>`;

        // 将<span>元素添加为欢迎信息的子元素，保证在div内部显示IP地址
        welcomeInfoElement.lastChild.appendChild(ipAddressElement);
    } catch (err) {
        console.log("Pjax无法获取元素");
    }
}
